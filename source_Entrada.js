...
function lista_arti() {
  ...
  $.ajax({
    ...
    url: "consulta_lista_arti.php",
    type: "POST",
    data: { fec_ini: fec_ini, fec_fin: fec_fin, tipo: 'E' },
    success: function (x) {
...
function genera_opcion_GE() {
  $(document).ready(function () {
    $.ajax({
      ...
      url: 'Mostrar_Fecha_Mercaderia.php',
      type: 'POST',
      data: 'option=' + 1,
...
function exportar_excel() {
  ...
  javascript: window.open("reporte_entrada_excel.php?fec_ini=" + fec_ini + "&fec_fin=" + fec_fin);
}
...
function busca_detalle() {
  id= $("#docito").val();
  // var id = document.getElementById('docito').value
      ...
      success: function (data) {
        ...
        if (global == 0) {
          ...
          $("#tabla_articulos > tbody > tr").each(function () {
            ...
            var can = $(this).closest('tr').find('td input[id="cantidad_item"]').val();
            console.log(can, 'da?');
            ...
            var preciou = $(this).closest('tr').find('td input[id="precio_uni"]').val();
            console.log(preciou);
            ...
            //   .val();
            var monto = $(this).closest('tr').find('td input[id="monto_item"]').val();
            console.log(monto);
            ...
            if (data == 0) {
              ...
              for (let i = 0; i < data.length; i++) {
                ...
                htmlString +=
                  ...
                  "</td>" +
                  "<td style='text-align:center;max-width: 40px;'><input type='text'  class='form-control pull-right' id='cantidad_item'  autocomplete='off' style='font-size: 12px; text-align:center; color:black; font-weight: bold;' value='0' onkeyup='calcular_total_item(this," + li + ")'  onchange='calcular_total_item(this," + li + ")' onkeypress='return validar_numero(event)' onselectstart='return false;' oncontextmenu='return false;'></td>" +
                  "<td style='text-align:right; max-width: 40px;'><input type='text'  class='form-control pull-right' id='precio_uni'  autocomplete='off' style='font-size: 12px; text-align:center; color:black; font-weight: bold;' value='" + data[i].precio_entrada + "' onchange='calcular_total_item(this," +
                  li + ")' onkeyup='calcular_total_item(this," + li + ")'onkeypress='return validar_numero(event)' ></td>" +

                  "<td style='text-align:center; max-width: 40px;'><input type='text'  class='form-control pull-right' id='monto_item'  autocomplete='off' style='font-size: 12px; text-align:center; color:black; font-weight: bold;' value='0' onkeyup='calcular_monto_tot(this," +
                  li +
                  ")'  onchange='calcular_monto_tot(this," +
                  li +
                  ")' onkeypress='return validar_numero(event)'></td>" +
                  "<td style='text-align:center;max-width: 60px;'> <select class='form-control' onchange='' data-placeholder='Seleccione un Almacen' style='width: 100%;font-size:12px;' tabindex='-1' aria-hidden='true' id='almacen'><option value='' >ELIJA ALMA.</option>"
