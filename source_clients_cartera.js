function lista_clientes2_kpi() {
  ...
  $(document).ready(function () {
    $.ajax({
      ...
      url: 'pone_clientes_cartera_kpi.php',
      type: 'POST',
      data: null,
      success: function (x) {
        ...
        $("#pone_clientes select").val('C99999999999').trigger('change.select2');
      },
...
function lista_proveedores2() {
  $.ajax({
    beforeSend: function () {
      $("#pone_clientes").html("Recuperando proveedores...");
    },
    url: 'pone_proveedores_cartera.php',
    type: 'POST',
    success: function (x) {
      $("#pone_clientes").html(x);
      $(".select2").select2();

      // 🔹 Una vez cargado el combo, añadimos el onchange
      const selectProveedor = document.getElementById("cliente");
      if (selectProveedor) {
        selectProveedor.onchange = function () {
          const codProveedor = this.value;
          if (codProveedor && codProveedor !== "-1" && codProveedor !== "") {
            lista_pedidoscompra(codProveedor);
          } else {
            // Limpia el combo de pedidos si no hay proveedor válido
            $("#pone_vendedores").html(
              "<select class='form-control select2'><option value='-1'>Seleccione un proveedor...</option></select>"
            );
            $(".select2").select2();
          }
        };
    ...
    error: function () {
      $("#pone_clientes").html("Error al cargar proveedores.");
    }
  });
}

// === Cargar pedidos de compra del proveedor ===
function lista_pedidoscompra(codProveedor) {
  $.ajax({
    beforeSend: function () {
      $("#pone_vendedores").html("Recuperando pedidos de compra...");
    },
    url: 'pone_pedidoscompra_proveedor.php',
    type: 'POST',
    ...
    error: function () {
      $("#pone_vendedores").html("Error al cargar pedidos (OC).");
    }
  });
}
...
function lista_estados_pedidos_ventas(doc) {
  //console.log('hola');
  $(document).ready(function () {
    $.ajax({
      beforeSend: function () {
        $("#pone_estados_pedidos_ventas").html("Recuperando proveedores...");
      },
      url: 'pone_estados_pedidos_ventas.php',
      type: 'POST',
      data: {
        doc: doc
      },
      success: function (x) {
        $("#pone_estados_pedidos_ventas").html(x);
        $(".select2").select2();
...
function lista_Marcas() {
  $(document).ready(function () {
    $.ajax({
      ...
      url: "Lista_Marcas_kp.php",
      type: "POST",
      data: null,
      success: function (x) {
        $("#lista_Marca").html(x);
        $("#lista_Marca select").val("-1").trigger("change")
        $(".select2").select2();
      },
      error: function (jqXHR, estado, error) { },
    });
...
function busca_cuentas_cliente() {
  ...
  if (!checkboxState) {
    ...
    $.ajax({
      ...
      type: 'POST',
      data: 'idcliente=' + $("#cliente").val() + '&tipo=' + checkboxState + '&idvendedor=' + $("#vendedor").val(), // Enviamos 1 cuando el checkbox está desmarcado
      success: function (x) {
        swal.close();
        $("#cartera_clientes").html(x);
        var table = $('#tabla_cliente_cartera').DataTable(
          {
        ...
        $("#tabla_cliente_cartera thead tr").clone(true).appendTo("#tabla_cliente_cartera thead");
        $("#tabla_cliente_cartera thead tr:eq(0) th").hide();

        $("#tabla_cliente_cartera thead tr:eq(1) th").each(function (i) {
          if (i == 2 || i == 5 || i == 6) {
            var $th = $(this);
            filter($th, table, i);
          }
        ...
        $("#mostrar-todos").on("click", function () {
          $('#tabla_cliente_cartera').DataTable().column(5).search("").draw();
        });
    ...
    $.ajax({
      ...
      url: 'consulta_cuenta_cliente.php',
      type: 'POST',
      data: 'idcliente=' + $("#cliente").val() + '&tipo=' + checkboxState + '&idvendedor=' + $("#vendedor").val(),// Enviamos 0 cuando el checkbox está marcado
      success: function (x) {
...
function lista_vendedores_kpi() {
  $(document).ready(function () {
    ...
    $.ajax({
      ...
      url: 'pone_vendedores_cartera_kpi.php',
      type: 'POST',
      data: null,
      success: function (x) {
        $("#pone_vendedores").html(x);
...
$(document).ready(function () {
  setTimeout(function () {
    $("#anio").val('2026').trigger('change.select2');
  }, 100);
});
  ...
  // setTimeout(() => {
    $(document).ready(function () {
      $.ajax({
        ...
        url: 'consulta_cuenta_vendedores.php',
        type: 'POST',
        data: 'idcliente=' + $("#pone_vendedores option:selected").val() + '&idmes=' + $("#cmeses").val() + '&anio=' + $("#anio").val(),
        success: function (x) {
