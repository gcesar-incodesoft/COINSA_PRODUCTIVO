...
function abrir_modal_detalle(id) {
  console.log(id);
  const [itemcode, descripcion, catalogo, marca] = id.split('|'); // Divide el ID en partes

  $("#codigo_articulo").val(itemcode);
  $("#nombre_articulo").val(descripcion);
  $("#catalogo_articulo").val(catalogo);
  $("#marca_articulo").val(marca);

  $("#modal_precio_historico3").modal("show");


  $.ajax({
    url: "consulta_pedido_pendientes_articulo.php",
    type: "POST",
