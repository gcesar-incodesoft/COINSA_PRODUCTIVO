...
function guarda_detart_mod() {
  var linea = parseInt($("#linea_det_mod").val(), 10);
  var linea2 = linea - 1;
  // console.log("Índice de línea limpio: ", linea);
  var detalle = $("#det_art_mod").val();
  // Selecciona la fila basada en 'linea', teniendo en cuenta que 'linea' ya es un índice basado en cero
  var fila = $("#tabla_art_ped tbody tr").eq(linea2);

  if (fila.length) {

    fila.find('td').eq(21).children("input").val(detalle);

    // fila.find("td:nth-child(" + columna + ")").text(detalle); // Insertar el detalle en la columna deseada
    $("#modal_detalle_art_mod").modal("hide"); // Ocultar el modal
  } else {
...
function pone_cliente(elid) {
  ...
  $("#idcliente_razon").val(idcl[1].replace(pattern, ""));
...
function procesa_cotizacion() {
  $(document).ready(function () {
    ...
    //return true; // Si todo está correcto, permite continuar
    let n_tic;
    ...
    $("#tabla_articulos > tbody > tr").each(function () {
      ...
      if (dscto_lin >= 10) {
        U_IMOB_ESTADO = "Pendiente de Autorizar";
        hay_descuento_alto = true;
      }
    ...
    // Si hay descuentos altos, pedir confirmación antes de continuar
    if (hay_descuento_alto) {
      ejecutarCotizacion1(n_tic, U_IMOB_ESTADO, band);
      //JD- SE COMENTO XQ NO APARECEN LAS OFS QUE NECESITAN APROBACION
      // Swal.fire({
      //   title: "Confirmación",
      //   text: "Algunos productos tienen más del 10% de descuento. ¿Desea proceder?",
      //   icon: "warning",
      ...
      //       text: "Se continuará con el registro.",
      //       icon: "success",
      //       showConfirmButton: false, // Oculta el botón
      //       timer: 4000 // Se cierra en 4 segundos
      //     });

      //     ejecutarCotizacion(n_tic, 'Pendiente', band);

      //     // Después de 2 segundos, ejecutar validaciones adicionales
      //     setTimeout(() => {
      //       correo_val(n_tic);
      //       // procesa_validacion(n_tic);
      //     }, 2000);
      ...
      //       showConfirmButton: false
      //     });
      //     band = false; // Detiene la ejecución
      //   }
      // });
    } else {
      // Si no hay descuentos altos, continuar con el proceso sin pedir confirmación
      ejecutarCotizacion1(n_tic, U_IMOB_ESTADO, band);
    }
...
function ejecutarCotizacion(n_tic, U_IMOB_ESTADO, band) {
  if (band === true) {
    ...
    direntrega = $("#cap_dicc").val();
    validez = $("#validez").val();
    dirigido_coti = $("#dirigido_coti").val();
    correo_dirigido = $("#correo_dirigido").val();
    telefono_dirigido = $("#telefono_dirigido").val();
    ...
    $("#tabla_articulos > tbody > tr").each(function () {
      ...
      tipo_iva = $(this).find("td").find("#tip_impuesto option:selected").val();
      if (tipo_iva === "IVAGPR19") {
        montoigv = parseFloat(montoigv + base * 0.19);
      }
      if (tipo_iva === "IVAZFR0") {
        montoigv = parseFloat(montoigv + base * 0.0);
      } else {
        montoigv = parseFloat(montoigv + 0);
      }
    });
    filas = $("#tabla_articulos").find("tbody tr").length;
    dscto = ""; //descuento total;
    ...
    $.ajax({
      ...
      data: {
        ...
        idcliente_parent: idcliente_parent,
        cpago: cpago,
        cmoneda: cmoneda,
        fentrega: fentrega,
        ...
        total_linea: subtotal_venta,
        n_ticket: parseInt(n_tic),
        father: fath,
        tipo_vta: tipo_venta,
        ...
        U_IMOB_OBSERVACION: U_IMOB_OBSERVACION,
        U_IMOB_OV: U_IMOB_OV,
        U_IMOB_FECOV: U_IMOB_FECOV,
        ...
        correo_dirigido: correo_dirigido,
        telefono_dirigido: telefono_dirigido,
        proyecto: proyecto,
        fdoc: fdoc,
        orden_compra_coti: orden_compra_coti,
        flete_cargo: flete_cargo

      },
      success: function (x) {
        let global = parseInt(x.trim());
        ...
        if (isNaN(global) || global <= 0) {
          ...
          $("#tabla_articulos > tbody > tr").each(function () {
            ...
            var fath = "";
            var modificado = "1";
            var detalle_art = $(this).find("td").find('input[id="det_arti"]').val();
            $.ajax({
              beforeSend: function () { },
              url: "procesa_cotizacion_det.php",
              type: "POST",
          ...
          swal("Se ha registrado la cotización", {
            icon: "success",
            buttons: false,
            timer: 2000,
          });
...
function ejecutarCotizacion1(n_tic, U_IMOB_ESTADO, band) {
  ...
  let cotizacionData = {
    ...
    correo_dirigido: $("#correo_dirigido").val(),
    contacto: $("#contactito option:selected").text(),
    telefono_dirigido: $("#telefono_dirigido").val(),
    cod_dire_entrega: $("#pone_cdireccion2 option:selected").val(),
    orden_compra_coti: $("#orden_compra_coti").val(),
    proyecto: $("#proyecto").val(),
  ...
  $("#tabla_articulos > tbody > tr").each(function () {
    ...
    let descripcion = $(this).find("td").find('input[id="descripcion"]').val() || "-";
    let marca = $(this).find("td").find('input[id="marca"]').val() || "-";
    let catalogo = $(this).find("td").find('input[id="catalogo"]').val() || "-";
    ...
    let plazo_entrega = quitarAcentos($(this).find("td").find('input[id="plazoEntrega_item"]').val());
...
function cerrar_cotizacion() {
  ...
  Swal.fire({
    ...
    text: "¿Desea cerrar la cotización?",
    icon: "warning",
    showCancelButton: true,
    confirmButtonText: "Sí",
    cancelButtonText: "No",
    ...
    if (result.isConfirmed) {
      ...
      Swal.fire({
        ...
        text: "Se ha cerrado la cotización",
        icon: "success",
        showConfirmButton: false,
        timer: 2000
      });
...
function duplica_data_articulo() {
  ...
  $("#tabla_art_ped > tbody > tr").each(function () {
    ...
    if (monto === "" || monto <= 0) {
      alertify.error("Linea: " + lina2 + " " + "Falta Precio Invalido");
      $(this).find("td").eq(10).css("background-color", "#F67280");
      band = false;
    } else {
      $(this).find("td").eq(10).css("background-color", "white");
    }
    if (dscto_lin.indexOf("+") !== -1) {
      alertify.error("Linea: " + lina2 + " " + "Falta Calcular Descuento");
      $(this).find("td").eq(9).css("background-color", "#F67280");
...
//   docentry = $("#docentry_cot").val();
//   ref_req_mod = $("#ref_req_mod").val();
//   validez_mod = $("#validez_mod").val();
//   cod_vendedor = $("#vendedor1").val();
...
//   band = true;
//   $('#tabla_art_ped > tbody > tr').each(function () {
//     subtotal_venta += parseFloat($(this).find("td").find('input[id="monto_item2"]').val());
//   });
...
//         $("#tabla_art_ped > tbody > tr").each(function () {
//           var n_orden = $(this).find("td").eq(1).html();
//           var line = $(this).find("td").eq(1).html();
//           var cod = quitarAcentos($(this).find("td").eq(3).html());
//           if (cod == "21004147" || cod == "21004146") {
//             var descripcion_art =
//               $(this).find("td").find('input[id="descripcion_mod2"]').val()
...
//             data: {
//               codigo: cod, descripcion_art: descripcion_art, cantidad: can, preciou: preciou, dscto_lin: dscto_lin,
//               total_linea: monto, tipo_vta: tipo_venta, father: fath, docentry: docentry, line: line, unidad_medida: unidad_medida,
//               plazo_entrega: plazo_entrega, marca: marca, catalogo: catalogo, modificado: modificado, multi_descuento: multi_descuento,
//               n_orden: n_orden, detalle_art: detalle_art
//             },
//             /*"&codigo=" +
//             cod +
//             "&descripcion_art=" +
...
//             modificado +
//             "&multi_descuento=" +
//             multi_descuento + "&n_orden=" + n_orden,*/
//             success: function (data) {
//               var n = noty({
//                 text: "Procesando venta...  articulo actual: " + cod,
//                 theme: "relax",
...
function actualizar_data_articulo() {
  ...
  $("#tabla_art_ped > tbody > tr").each(function () {
    ...
    let plazo_entrega = $(this).find("td").find('input[id="plazoEntrega_item2"]').val();
    let detalle_art = 
                   $(this).find("td").find('input[id="detalle_art2"]').val()
...
function add_art_mod() {
  Swal.fire({
    title: "¿Estás seguro?",
    text: "¿Quieres proceder con la selección de artículos?",
    icon: "warning",
    showCancelButton: true,
    ...
    if (result.isConfirmed) {
      // Si el usuario confirma, ejecutar la función normalmente
      $("#modal_busqueda_arts_mod").modal("toggle");

      let line = [];
      $('#tabla_art_mod input[type="checkbox"]:checked').each(function () {
        let codigo = $(this).closest("tr").children("td:eq(1)").text();
        line.push(codigo);
      });

      let codigo = line.toString();
      $("#codigo2").val(codigo);
      busca_articulo_mod();
    }
  });
            ...
            if (data == 0) {
              ...
              for (let i = 0; i < data.length; i++) {
                ...
                htmlString +=
                  ...
                  precio +
                  "</td>" +
                  "<td style='text-align:center'><input type='text' class='form-control pull-right' id='detalle_art2' autocomplete='off' style='font-size: 12px; text-align:center; color:black; font-weight: bold;' value=''></td>";
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      Swal.fire({
        ...
        text: "Cuando le dé actualizar, se eliminará.",
        icon: "success",
        showConfirmButton: false,
        timer: 2000
      });
        ...
        if (valido == "false") {
          ...
          if (firstTwoDigits === "20") {
            ...
            obtenerDatosJson(function (data) {
              if (data) {
                const nombreABuscar = data2.result.departamento;
                ...
                if (infoUbigeo) {
                  ...
                  setTimeout(() => {
                    $("#pone_dpto_cli select")
                      .val(infoUbigeo.codigo_ubigeo)
                      .trigger("change");
                    obtenerDatosJson2(function (data) {
                      const idBuscado = infoUbigeo.id_ubigeo;
                      const nombreBuscado = data2.result.provincia;
                      if (data) {
                        const resultadoBusqueda = buscarPorIdYNombre(
                          idBuscado,
                          nombreBuscado,
                          data
                        );
                        ...
                        if (resultadoBusqueda) {
                          data_ubig =
                            infoUbigeo.codigo_ubigeo +
                            resultadoBusqueda.codigo_ubigeo;
                          setTimeout(() => {
                            $("#pone_prov_cli select")
                              .val(data_ubig)
                              .trigger("change");
                            obtenerDatosJson3(function (data) {
                              const idBuscado2 = resultadoBusqueda.id_ubigeo;
                              const nombreBuscado2 = data2.result.distrito;
                              if (data) {
                                const resultadoBusqueda2 = buscarPorIdYNombre(
                                  idBuscado2,
                                  nombreBuscado2,
                                  data
                                );
                                ...
                                if (resultadoBusqueda2) {
                                  data_ubig2 =
                                    infoUbigeo.codigo_ubigeo +
                                    resultadoBusqueda.codigo_ubigeo +
                                    resultadoBusqueda2.codigo_ubigeo;
                                  setTimeout(() => {
                                    $("#pone_dist_cli select")
                                      .val(data_ubig2)
                                      .trigger("change");
                                  }, 1000);
          ...
          $("#agente_ret").val("NO").trigger("change");
          ...
          for (var i = 0; i < data2.result.establecimientos.length; i++) {
            var tr =
              `<tr>
              <td  class='center'>` +
              data2.result.establecimientos[i].codigo +
              `</td>
              <td  class='center'>` +
              data2.result.establecimientos[i].cod_tipo +
              `</td>
              <td  class='center'>` +
              data2.result.establecimientos[i].tipo +
              `</td>
              <td  class='center'>` +
              data2.result.establecimientos[i].direccion +
              `</td>
              <td  class='center'>` +
              data2.result.establecimientos[i].departamento +
              `</td>
              <td  class='center'>` +
              data2.result.establecimientos[i].provincia +
              `</td>
              <td  class='center'>` +
              data2.result.establecimientos[i].distrito +
              `</td>
              <td  class='center'>` +
              data2.result.establecimientos[i].activida_economica +
              `</td>
              <td class='center'>
          ...
          agente_retencion = data2.result.agente_retencion;
          if (agente_retencion !== "") {
            agente_retencion2 = agente_retencion.split(",");
            //console.log(agente_retencion2);
            $("#agente_ret").val(agente_retencion2[0]).trigger("change");
          }
          agente_percepcion = data2.result.agente_perc_vta_int;
          if (agente_percepcion !== "") {
            agente_percepcion2 = agente_percepcion.split(",");
            //console.log(agente_percepcion2);
            $("#agente_per").val(agente_percepcion2[0]).trigger("change");
          }
          if (data2.result.estado === "ACTIVO") {
            $("#estado_ruc").val("01").trigger("change");
          }
          if (data2.result.condicion === "HABIDO") {
            $("#condicion_ruc").val("01").trigger("change");
          }
          if (data2.result.buen_contribuyente === "SI") {
  ...
  if (bandera === true) {
    ...
    $.post(
      ...
      function (dat2) {
        ...
        //console.log(global);
        if (global == 0) {
          Swal.fire("No Insertado!", "Cliente no insertado", "error");
        } else if (global == -1) {
          Swal.fire("No Insertado!", "Cliente existe en la WEB", "error");
        } else if (global == -2) {
          Swal.fire("No Insertado!", "Cliente existe en SAP", "error");
        } else {
          ...
          Swal.fire("¡Insertado!", "Cliente insertado", "success");
            ...
            if (data == 0) {
              ...
              for (let i = 0; i < data.length; i++) {
                ...
                htmlString +=
                  ...
                  precio +
                  "</td>" +
                  "<td style='text-align:center'><input type='text' class='form-control pull-right' id='detalle_art' autocomplete='off' style='font-size: 12px; text-align:center; color:black; font-weight: bold;' value=''></td>";
...
function pasar_pendiente2(docentry) {
  ...
  Swal.fire({
    ...
    text: "¿Desea procesar cotización?",
    icon: "warning",
    showCancelButton: true,
    confirmButtonText: "Sí",
    cancelButtonText: "No",
    ...
    if (hasHighDiscount) {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          $.post(
            "procesa_cotizacion_creada4.php",
            {
              codigo: doc_cotiza,
            },
            function (data1) {
...
function pasar_a_pedido(docentry) {
  ...
  setTimeout(() => {
    ...
    consultar_data_proc_det(idcl[0]);
    tipo_cambio_hoy3();
...
function lista_direccion_condicion_mod() {
  var idcliente3 = "";
  idcliente3 = $("#idcliente_credito").val();
  $(document).ready(function () {
    $.ajax({
...
function actualizarCantidadKit(input) {
  let kitGroup = input.getAttribute('data-kit'); // Obtiene el grupo del kit
  let nuevaCantidad = input.value;

  if (kitGroup && kitGroup !== "0") {
    document.querySelectorAll(`input[data-kit='${kitGroup}']`).forEach((elemento) => {
      elemento.value = nuevaCantidad; // Actualiza la cantidad en todos los componentes del kit
    });
  }
}
function bloquear_fila(button) {
  Swal.fire({
    title: "¿Estás seguro?",
    text: "Esta acción bloqueará la fila y no podrás editarla.",
    icon: "warning",
    showCancelButton: true,
    ...
    if (result.isConfirmed) {
      let row = $(button).closest("tr"); // Obtener la fila

      // Cambiar el estado a 'C' en la celda correspondiente
      row.find("td[data-status]").text("C");

      // Deshabilitar todos los inputs y selects dentro de la fila
      row.find("input, select, textarea").prop("disabled", true);

      // Cambiar el color de fondo a gris claro (SAP-style)
      row.css("background-color", "#e0e0e0");

      Swal.fire("Bloqueado", "La fila ha sido bloqueada correctamente.", "success");
    }
  });
}
function procesa_validacion(n_tic) {
  $.post(
