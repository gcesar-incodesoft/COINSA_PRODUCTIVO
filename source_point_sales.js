...
//            alert("Parece ser que hay un error por favor, reportalo a Soporte inmediatamente...!");
 ...
 function agrega_a_lista(){
   $(document).ready(function(){
            ...
            $("#tabla_articulos > tbody").append("<tr><td class='center'>"+articulo+"</td><td class='center'>"+descripcion+"</td><td class='center'>"+cantidad+"</td><td class='center'>"+precio+"</td><td class='center'>"+dsctoline+"</td><td class='center'>"+monto.toFixed(2)+"</td><td class='center' style='display:none'>"+tipovta+"</td><td class='center' style='display:none'>"+articulo+"</td><td class='center'><button class='btn btn-block btn-danger btn-xs delete'><i class='icon-trash bigger-120'></i> Eliminar</button></td></tr>");
            $("#codigo").val("");
            $("#cantidad").val(0.00);
            $("#dsctoline").val(0.00);
				$("#monedaitem").val("");
...
var i = $(this).index();
    ...
    cantidad=parseFloat($(this).find('td').eq(2).html());
    precio=parseFloat($(this).find('td').eq(3).html());
	tipoarticulo=$(this).find('td').eq(6).html();
        dsctoline=parseFloat($(this).find('td').eq(4).html());
    if(isNaN(dsctoline)) {
         dsctoline = 0.00;
...
function pone_num_venta(){
          ...
          success: function(x){
            ...
            $("#num_ticket").html("Sesión: "+$("#ncaja").val()+" - Pedido # " +x);
           },
           error: function(jqXHR,estado,error){
