...
$(document).ready(function () {
    $("#busca_idr").on("keypress", function (e) {
        if (e.which === 13) {
...
function busca_Lista_Despachos() {
    setTimeout(() => {
        var fechai = $("#fechai").val();
        var fechaf = $("#fechaf").val();
        var conductor = $("#lista_conductor select").val();
        var placas = $("#lista_placas select").val();
        var idr = $("#busca_idr").val();

        $.ajax({
            beforeSend: function () {
                swal_carga()
            },
            url: 'Lista_control_alma_despachados_ped.php',
            type: 'POST',
            data: {
                fechai: fechai,
                fechaf, fechaf,
...
function genera_excel_pedidos() {
    ...
    javascript: window.open("reporte_excel_gestion_pedidos.php?fechai=" + fechai + "&fechaf=" + fechaf + "&conductor=" + conductor + "&placas=" + placas + "&idr=" + idr);
}
...
function lista_conductor() {
    $(document).ready(function () {
        $.ajax({
            beforeSend: function () {
                $("#lista_conductor").html("Recuperando Lista ...");
            },
            url: "listado_conductor_rep.php",
            type: "POST",
            data: null,
            success: function (x) {
                $("#lista_conductor").html(x);
                $(".select2").select2();
...
function lista_unidad_rep() {
    $(document).ready(function () {
        $.ajax({
            beforeSend: function () {
                $("#lista_placas").html("Recuperando Lista ...");
            },
            url: "listado_unidad_rep.php",
            type: "POST",
            data: null,
            success: function (x) {
                $("#lista_placas").html(x);
...
function bloquearForm4() {
    ...
    document.getElementById("id_moda_destino_pro4").disabled = true;
    document.getElementById("id_moda_pago").disabled = true;
...
function bloquearForm5() {
    ...
    document.getElementById("id_moda_destino_pro5").disabled = true;
    document.getElementById("id_moda_pago_pro5").disabled = true;
...
function bloquearForm_pro() {
    ...
    document.getElementById("id_moda_destino_pro").disabled = true;
    document.getElementById("id_moda_pago_pro").disabled = true;
...
function guarda_actualiza_form4() {
    ...
    if (id_moda_destino === '-1') {
        bandera = false;
        alertify.error("Seleccione la modalidad de destino");
        $("#modalidad_destino_pro4").focus();
    }
    if (id_moda_pago === '-1') {
        bandera = false;
        alertify.error("Seleccione la modalidad de pago");
        $("#id_moda_pago_pro4").focus();
    }
    ...
    $("#miTabla_pro4_2 > tbody > tr").each(function () {
        comta2 = $(this).find("td").find('input[id="conta2"]').val();
        dni2 = $(this).find("td").find('input[id="dni2"]').val();
        tele2 = $(this).find("td").find('input[id="tele2"]').val();
    });
    ...
    $("#miTabla_pro4_3 > tbody > tr").each(function () {
        comta3 = $(this).find("td").find('input[id="conta3"]').val();
        dni3 = $(this).find("td").find('input[id="dni3"]').val();
        tele3 = $(this).find("td").find('input[id="tele3"]').val();
    });
...
function guarda_actualiza_form5() {
    ...
    if (id_moda_destino === '-1') {
        bandera = false;
        alertify.error("Seleccione la modalidad de destino");
        $("#modalidad_destino_pro5").focus();
    }
    if (id_moda_pago === '-1') {
        bandera = false;
        alertify.error("Seleccione la modalidad de pago");
        $("#id_moda_pago_pro5").focus();
    }
...
function actualizar_form_pro() {
    ...
    if (id_moda_destino === '-1') {
        bandera = false;
        alertify.error("Seleccione la modalidad de destino");
        $("#modalidad_destino_pro").focus();
    }
    if (id_moda_pago === '-1') {
        bandera = false;
        alertify.error("Seleccione la modalidad de pago");
        $("#id_moda_pago_pro").focus();
    }
...
function imprimi_ruta_directo() {
    ...
    var url = "pdf_impresion_ruta_pedidos.php?fechai=" + fechai + "&fechaf=" + fechaf + "&conductor=" + conductor + "&placas=" + placas + "&idr=" + idr;



    $('#modal_data_pdf_control_ruta').modal('show');
    $('#modal_data_pdf_control_ruta').on('shown.bs.modal', function () {
    ...
    $("#imprimir_1").off('click').on('click', function () {
        $('#modal_data_pdf_control_ruta').find('iframe')[0].contentWindow.print();
    });
...
function imprimi_ruta_directo_indi(id) {
    ...
    var url = "pdf_impresion_ruta_linea.php?idr=" + idr + "&fecha=" + fecha + "&unidad=" + unidad + "&conductor=" + conductor + "&ayudante=" + ayudante + "&doc_guia=" + doc_guia;
...
function imprimir_r1_Des() {
    ...
    if (cant === '') {
        ...
        alertify.error("El valor no puede ser mayor a " + cantidad_fija);
        // swal("El valor no puede ser mayor a " + cantidad_fija, {
        //     icon: "warning",
        //     timer: 2000,
        //     buttons: false,
        // });
...
function registrar_ruta() {
    ...
    if (bandera === true) {
        ...
        $.ajax({
            beforeSend: function () {
                // Se puede agregar un swal de carga aquí
            },
...
$(document).on("dblclick", "#tabla_listado_despachados tbody tr", function () {
    ...
    var nom_estado = $fila.find("td:nth-child(8)").text();
...
$(document).on("dblclick", "#tabla_listado_finalizados tbody tr", function () {
    ...
    var guia = $fila.find("td:nth-child(7)").text().trim();
