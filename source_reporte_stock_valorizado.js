...
function pone_Familia() {
  $(document).ready(function () {
    $.ajax({
      beforeSend: function () {
        $("#pone_Familia_entrada").html("Recuperando articulos...");
      },
...
function lista_arti() {
  ...
  $.ajax({
    ...
    url: "consulta_lista_arti_stock_valorizado.php",
    type: "POST",
...
function exportar_excel_stock_val() {
  fami = $("#pone_Familia_entrada option:selected").val();
  subfami = $("#pone_SubFamilia option:selected").val();
  subfami2 = $("#pone_SubFamilia2 option:selected").val();
  marca = $("#lista_Marca option:selected").val();
  ...
  javascript: window.open(
    "reporte_excel_stock_valorizado.php?&fami=" +
    fami +
    "&subfami=" +
    subfami +
    "&subfami2=" +
