...
function lista_cmeses() {
  $(document).ready(function () {
    ...
    $.ajax({
      ...
      success: function (x) {
        $("#pone_cmeses").html(x);
        $('#pone_cmeses select').val('-1').trigger("change.select2")
...
function busca_resumen_ventas() {

  setTimeout(() => {
    $(document).ready(function () {
      $.ajax({
        beforeSend: function () {
          // $("#lista_top_ventas").html("Consultando ventas...");
          // $("#lista_top_ventas").html('<div class="spinner-container"><i class="fas fa-spinner fa-spin"></i> </div>');
          swal({
            title: "Cargando...",
        ...
        error: function (jqXHR, estado, error) {
          $("#lista_top_ventas").html(estado + "    " + error);
        }
      });
    });
  }, 500);
